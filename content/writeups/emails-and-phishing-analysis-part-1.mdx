---
title: "Emails and Phishing Analysis (Part-1)"
date: '2024-08-06'
tags: ['email-analysis', 'phishing', 'blue-teaming', 'part-1']
writtenBy: "2"
summary: "Learn how to tackle malicious emails."
cover: "/images/writeups/email_phishing.png"
---

## Phishing ?

It is a type of social engineering attack, it is a serious attack vector that you, as a defender, will have to defend against.
...
The invention of email dates back to the 1970s for ARPANET (United States defense program, experimental computer networks that was forerunner of the Internet).
...
2. @ (e.g., works as a separator of the Username and Domain)
3. Domain (e.g., in the example hotmail.com)

So, basically the username means whom the email is to be sent to and domain refers to the service the user is using like Gmail, Yahoo or Hotmail etc.
Or you can consider the domain as your street number and username is the person's first name or last name.

## Email Delivery

Moving further, how email is delivered from one person to another. A handful of protocols are involved in sending the email.
...
3. The DNS server obtains the information for johndoe.com and sends that information to the SMTP server.
...
6. Alexa's email finally reaches the destination SMTP server.
...
So, lastly every protocol has its unique/associated port number. So, the port numbers for the above protocols are given below:
...
The email body is the part of an email which contains the text (plain or HTML formatted) the sender wants you to view.

This is example of a text-only email.

![image](https://miro.medium.com/v2/resize:fit:4800/format:webp/1*yDL-adZwrWFbkAR1fKOYDA.png)

And below is an example of the HTML formatted email.

![image](https://miro.medium.com/v2/resize:fit:1100/format:webp/1*AyBXPblLcgN5EBulyv4DZg.png)
...
![image](https://miro.medium.com/v2/resize:fit:4800/format:webp/1*k0AZ-F7h8M3lyqqV7y_qig.png)

A snippet of the HTML code is shown below.

![image](https://miro.medium.com/v2/resize:fit:4800/format:webp/1*dcv_9KYQNIeFK0P0GboMRA.png)

In this specifc email web client, Proton-mail, the option to switch back to HTML is called "**View rendered HTML**".

![image](https://miro.medium.com/v2/resize:fit:1100/format:webp/1*DqqXz5mLekRy_sxnPhFyFQ.png)
...
![image](https://miro.medium.com/v2/resize:fit:1100/format:webp/1*1harlIZj7YmITXU8oTbB5g.png)
...
![image](https://miro.medium.com/v2/resize:fit:4800/format:webp/1*5Wy1sBtgW09K2Yu3htTAjw.png)
...
- Phishing: Emails sent to target(s) purporting to be from a trusted entity to lure individuals into providing sensitive information.
...
- The sender's email name/address will masquerade as a trusted entity [email spoofing](https://www.proofpoint.com/us/threat-reference/email-spoofing).
